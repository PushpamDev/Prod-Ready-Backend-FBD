...
/**
 * UPDATED: Now identifies "Incomplete" batches where faculty missed logs.
 */
const getFacultyAttendanceReport = async (req, res) => {
  const { facultyId } = req.params;
  const { locationId } = req; // Assuming auth middleware sets this
  ...
  try {
    ...
    // Fetch batches where faculty is either permanent or a substitute
    ...
    if (involvedBatchIds.length === 0) return res.json({ faculty_id: facultyId, faculty_name: facultyData.name, faculty_attendance_percentage: 0, batches: [] });
    ...
    const batchReports = activeBatches.map(batch => {
      ...
      const expectedDates = getExpectedSessionDates(batch.start_date, batch.end_date, batch.schedule);
      
      // Filter attendance to only count days this specific faculty was responsible (considering substitutions)
      ...
      const presentCount = relevantAttendance.filter(a => a.is_present).length;
      const totalPossible = studentCount * markedDates.length;
      const missingDates = expectedDates.filter(d => !markedDates.includes(d));

      totalPresentGlobal += presentCount;
      ...
      return {
        ...
        total_sessions: markedDates.length,
        attendance_percentage: totalPossible > 0 ? parseFloat(((presentCount / totalPossible) * 100).toFixed(2)) : 0,
    ...
    res.status(200).json({
      ...
      faculty_attendance_percentage: totalPossibleGlobal > 0 ? parseFloat(((totalPresentGlobal / totalPossibleGlobal) * 100).toFixed(2)) : 0,
      batches: batchReports,
      missing_logs: batchReports.filter(b => !b.compliance.is_complete).map(b => ({ batch_name: b.batch_name, count: b.compliance.missing_attendance_dates.length }))
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const getOverallAttendanceReport = async (req, res) => {
  try {
    if (!req.locationId) return res.status(401).json({ error: 'Auth required.' });

    // 1. Fetch base data for the location
    const { data: faculties } = await supabase.from("faculty").select("id, name").eq('location_id', req.locationId);
    const { data: batchesData } = await supabase.from("batches").select("id, name, faculty_id, start_date, end_date, schedule").eq('location_id', req.locationId);
    ...
    // 2. Fetch all related records for active batches
    ...
    const facultyReports = Object.keys(facultyStats).map(fId => {
        ...
        activeBatches.forEach(batch => {
            ...
            if (isPrimary || hasMarkedData) {
                ...
                /** * DISCREPANCY FIX: Math Guard 
                 * We cap the present count by (students * sessions).
                 * This prevents attendance from exceeding 100% if "ghost" records exist.
                 */
                ...
                // Compliance logic (Only for primary owner)
                const expectedDates = getExpectedSessionDates(batch.start_date, batch.end_date, batch.schedule);
    ...
    // 6. Return response with Institute-wide Average
    res.status(200).json({ 
